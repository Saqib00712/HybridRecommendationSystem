"""
Seed 50 realistic tech courses into database with real embeddings
"""
import asyncio
...
courses = [
    ...
    {"title": "Python for Beginners", "description": "Learn Python programming from scratch. Covers basics to advanced concepts.", "category": "Python", "difficulty": "Beginner", "price": 49.99, "instructor": "John Doe", "duration": "8 weeks", "tags": "python,programming,beginners,coding"},
]


def seed_products():
    """Add all 50 courses with REAL embeddings"""
    ...
    for i, course in enumerate(courses):
        ...
        embedding = loop.run_until_complete(generate_embedding(text_for_embedding))
        loop.close()
        ...
        added_count += 1
        print(f"✅ [{added_count}/50] {course['title']} ({len(embedding)} dims)")
    
    db.commit()
    db.close()
    
    print(f"\n🎉 Added {added_count} products with real embeddings!")
    print(f"📊 Total: {existing_count + added_count} products")
